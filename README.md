...
```bat
pip install slack-bolt slack-sdk python-dotenv psutil requests pyautogui Pillow mcp pywin32 selenium
```
...
2. Scroll to **Scopes** → **Bot Token Scopes**, click **Add an OAuth Scope**, and add:
   ...
   - `files:read` — download images sent by users
   - `files:write` — upload screenshots/images to Slack
   - `groups:history` — read messages in private channels
   - `im:history` — read direct messages
   - `im:read` — view DM info
   - `im:write` — open DMs
   - `users:read` — look up user information
   - `users:read.email` — look up users by email (for allowlist)
